...
let popupBindingsState: ReturnType<typeof buildPopupBindingsState>;
...
vi.mock("./components/DashboardPopups", () => ({
  DashboardPopups: () => <div data-testid="dashboard-popups" />,
  loadCreateProjectPopupModule: () => Promise.resolve(),
  loadCreateWorkspacePopupModule: () => Promise.resolve(),
  loadSearchPopupModule: () => Promise.resolve(),
  loadSettingsPopupModule: () => Promise.resolve(),
}));
...
vi.mock("./components/DashboardChrome", () => ({
  DashboardChrome: ({
    ...
    <div>
      <div data-testid="dashboard-chrome">{String(canCreateWorkspace)}</div>
      <button type="button" onClick={onCreateWorkspace}>Create Workspace</button>
      <button type="button" onClick={() => onSwitchWorkspace("secondary")}>Switch Workspace</button>
      <button type="button" onClick={handleSettingsIntent}>Open Settings</button>
    </div>
  ),
}));
...
vi.mock("./components/DashboardContent", () => ({
  DashboardContent: ({
    ...
    contentModel: { kind: string };
    handleNavigateToArchiveProject: (projectId: string) => void;
    createMainContentProjectActions: (projectId: string) => { updateProject?: (data: { description: string }) => void };
    baseMainContentNavigationActions: { navigate?: (view: AppView) => void };
  }) => (
    <div>
      <div data-testid="dashboard-content">{contentModel.kind}</div>
      <button type="button" onClick={() => handleNavigateToArchiveProject("project-1")}>Go Archive Project</button>
      <button
        type="button"
        onClick={() => createMainContentProjectActions("project-1").updateProject?.({ description: "Updated" })}
      >
        Update Project
      </button>
      <button type="button" onClick={() => baseMainContentNavigationActions.navigate?.("archive")}>Navigate Archive</button>
    </div>
  ),
}));
...
beforeEach(() => {
  ...
  popupBindingsState = buildPopupBindingsState();
  useDashboardPopupBindingsMock.mockReturnValue(popupBindingsState);
...
describe("DashboardShell", () => {
  test("renders a loading state when snapshot is unavailable", () => {
  ...
  test("renders chrome, popups, and content once snapshot is loaded", () => {
    render(<DashboardShell />);

    expect(screen.getByTestId("dashboard-popups")).toBeInTheDocument();
    expect(screen.getByTestId("dashboard-chrome")).toHaveTextContent("true");
    expect(screen.getByTestId("dashboard-content")).toHaveTextContent("tasks");

    fireEvent.click(screen.getByRole("button", { name: "Switch Workspace" }));
    fireEvent.click(screen.getByRole("button", { name: "Create Workspace" }));
    fireEvent.click(screen.getByRole("button", { name: "Open Settings" }));
    fireEvent.click(screen.getByRole("button", { name: "Go Archive Project" }));
    fireEvent.click(screen.getByRole("button", { name: "Update Project" }));
    fireEvent.click(screen.getByRole("button", { name: "Navigate Archive" }));

    expect(dashboardCommandsState.workspace.switchWorkspace).toHaveBeenCalledWith("secondary");
    ...
    expect(navigationState.navigateView).toHaveBeenCalledWith("archive");
    expect(popupBindingsState.handleSettingsIntent).toHaveBeenCalledTimes(1);
    expect(projectActionsState.handleUpdateProject).toHaveBeenCalledWith("project-1", { description: "Updated" });
  });

  test("marks workspace creation unavailable for non-owner roles", () => {
    ...
    expect(screen.getByTestId("dashboard-chrome")).toHaveTextContent("false");
  });
});
