...
export function SidebarHelpActions() {
  const [activePopup, setActivePopup] = useState<"feedback" | "bug" | null>(
    null,
  );
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    toast.success(
      activePopup === "feedback" ? "Feedback sent!" : "Bug report submitted!",
    );
    setActivePopup(null);
  };
  return (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          ...
          <div className="w-full max-w-[400px] bg-[#181818] border border-[#262626] rounded-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 relative z-10">
            ...
            <form onSubmit={handleSubmit} className="p-4 flex flex-col gap-4">
              <textarea
                className="w-full h-[120px] bg-[#121212] border border-white/10 rounded-lg p-3 txt-role-body-md text-white focus:outline-none focus:border-white/20 placeholder:text-white/20 resize-none font-app"
                placeholder={
              ...
              <div className="flex justify-end">
                <button
                  type="submit"
                  className="px-3 py-1.5 bg-white text-black txt-role-body-md font-medium rounded-lg hover:bg-white/90 transition-colors flex items-center gap-2"
                >
                  <span>Submit</span> <Send size={12} />
                </button>
              </div>
            </form>
          </div>
