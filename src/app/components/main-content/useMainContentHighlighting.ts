...
export function useMainContentHighlighting({
  ...
  useEffect(() => {
    ...
    if (
      pendingHighlight.projectId &&
      pendingHighlight.projectId !== projectId
    ) {
      return;
    }
    ...
    if (pendingHighlight.type === "file" && pendingHighlight.fileName) {
      if (
        pendingHighlight.fileTab &&
        isProjectFileTab(pendingHighlight.fileTab)
      ) {
      ...
      const file =
        projectFiles.find(
          (entry) =>
            ...
            (!pendingHighlight.fileTab ||
              entry.tab === pendingHighlight.fileTab),
        ) ??
