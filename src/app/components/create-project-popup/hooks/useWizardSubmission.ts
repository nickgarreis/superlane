...
export const useWizardSubmission = ({
  ...
  const hasUnsavedWork = useCallback(() => {
    if (!editProjectId) {
      return Boolean(selectedService) || Boolean(projectName.trim()) || attachments.length > 0;
    }
    ...
    if (initialDraftData) {
      ...
      if (Boolean(isAIEnabled) !== Boolean(initialDraftData.isAIEnabled)) return true;
  ...
  const handleConfirmDelete = useCallback(() => {
    if (editProjectId && onDeleteDraft) {
      void Promise.resolve()
        ...
        .catch((error) => {
          reportUiError("createProjectWizard.deleteDraft", error, { showToast: false });
